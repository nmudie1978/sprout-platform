...
// Goal Card Component (simplified from original)
function GoalCard({
  goal,
  slot,
  isEditing,
    ...
    return (
      ...
      >
        ...
        <CardContent className="space-y-4">
          ...
          <div className="grid grid-cols-3 gap-3">
            <div>
              ...
              <Select
                ...
                onValueChange={(v) => setEditForm({ ...editForm, status: v as GoalStatus })}
              >
                <SelectTrigger>
                  <SelectValue />
                </SelectTrigger>
            ...
            <div>
              ...
              <Select
                ...
                onValueChange={(v) => setEditForm({ ...editForm, timeframe: v as GoalTimeframe })}
              >
                <SelectTrigger>
                  <SelectValue />
                </SelectTrigger>
          ...
          <div>
            <label className="text-sm font-medium mb-1 block">Why this goal?</label>
            <Textarea
              value={editForm.why}
  ...
  return (
    ...
    >
      ...
      <CardContent className="space-y-4">
        ...
        <GoalPodcasts goalTitle={goal!.title} isPrimary={isPrimary} />

        {isPrimary && <WorkplaceInsightPlaceholder goalTitle={goal!.title} />}
      </CardContent>
    </Card>
  );
}
...
// Main Goals Content Component (exported for use in My Journey page)
export default function GoalsPageContent() {
  ...
  const updateGoalMutation = useMutation({
    mutationFn: async ({ slot, goal }: { slot: GoalSlot; goal: CareerGoal | null }) => {
      const response = await fetch("/api/goals", {
        method: "PUT",
  ...
  const deleteGoalMutation = useMutation({
    mutationFn: async (slot: GoalSlot) => {
      const response = await fetch(`/api/goals?slot=${slot}`, { method: "DELETE" });
  ...
  const handleAddNoteFromSheet = useCallback(
    (content: string) => {
