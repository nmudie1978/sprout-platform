  ...
  return (
    <DropdownMenu>
      <DropdownMenuTrigger
        ...
        className={cn(
          iconOnly
            // Real size on touch rather than `.hit-44`: this trigger sits beside the
            // dashboard's walkthrough button on a gap-2 row, so a projected 44px
            // area would overlap it and (being the later sibling) steal its taps.
            ? "h-9 w-9 sm:h-7 sm:w-7 rounded-pill border border-border/40 bg-background/60 flex items-center justify-center text-muted-foreground/70 hover:text-foreground hover:bg-muted/40 transition-colors"
